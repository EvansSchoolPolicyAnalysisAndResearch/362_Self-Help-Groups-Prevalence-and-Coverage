...
*Title/Purpose 	: This do.file was developed by the Evans School Policy Analysis & Research Group (EPAR) 
				  ...
				  Tanzania Financial Inclusion Insights (FII) survey Wave 4 (2016) 											
...
*Data source
*-----------
*The Tanzania Financial Inclusion Insights survey was collected by InterMedia 
*over the period August - September 2016.
...
*-----------
*This master do file constructs selected indicators using the Tanzania FII data set.
...
global TZ_FIIW4_data 							"FII/Raw Data"
...
*CLEAN DATA
************
use "$TZ_FIIW4_data\Tanzania Wave 4 Stata.dta", clear
keep Serial DG2 DG5* AA3 AA7 poverty ppi* UR /* relevant demographic data
*/ DL28 /* emergency
...
*Mobile Money
ren MM2_1 aware_vodacom
ren MM2_2 aware_tigo
ren MM2_3 aware_airtel
ren MM2_4 aware_zantel
ren MM2_5 aware_SMARTB
ren MM2_6 aware_halo
ren MM4_1 use_vodacom
ren MM4_2 use_tigo
ren MM4_3 use_airtel
ren MM4_4 use_zantel
ren MM4_5 use_SMARTB
ren MM4_6 use_halo
*Official ID
ren DG5_1 id_gov
...
ren FB16_4 loan_SACCO	//Savings and Credit Cooperative
...
replace MM_use = 0 if aware_vodacom!=. | aware_tigo!=. | aware_airtel!=. | aware_zantel!=. | aware_SMARTB!=. | aware_halo!=. 
replace MM_use = 1 if use_vodacom==1 | use_tigo==1 | use_airtel==1 | use_zantel==1 | use_SMARTB==1 | use_halo==1 
...
replace coop_fin_serv = 0 if acct_coop==2 //IFI3_2 = cooperative acct 
...
lab var SACCO_int_rt_loan "Individual knows interest rate of their SACCO loan"
...
lab var SACCO_int_rt_sav "Individual knows interest rate of their SACCO savings account"
...
gen ASCA_int_rt_sav = . 
lab var ASCA_int_rt_sav "Individual knows interest rate of their ASCA/VSLA savings account"
...
*Saving
save "$created_data\TZ_FII_final.dta", replace

************
*Disaggregate and Export
************ 
use "$created_data\TZ_FII_final.dta", clear
...
*Create global of users vs. non-user indicators
	*ASCA, Cooperative (unspecified), Merry-Go-Round, SACCO, Savings Group, SHG (unspecified)
	gen ASCA_use_any = ASCA_fin_serv
	gen ASCA_no_use = .
	replace ASCA_no_use = 1 if ASCA_use_any == 0 
	replace ASCA_no_use = 0 if ASCA_use_any == 1
	gen savings_group_use_any = SHG_membership
	gen savings_group_no_use = .
	replace savings_group_no_use = 1 if savings_group_use_any == 0
	replace savings_group_no_use = 0 if savings_group_use_any == 1
	gen merry_no_use = . 
...
*/ savings_group_use_any savings_group_no_use
...
*Coverage indicators
putexcel set "$final_data\TZ_FII_estimates.xls", modify sheet("Coverage", replace)
...
foreach v of global coverage_indicators {
	...
	putexcel 	A`row'="FII" C`row'="Coverage" I`row'="1.1 All individuals" ///
...
putexcel set "$final_data\TZ_FII_estimates.xls", modify sheet("Characteristics", replace)
...
putexcel set "$final_data\TZ_FII_estimates.xls", modify sheet("Users vs. Non-Users", replace)
...
foreach v of global disagg_user_nonuser {		
	foreach x of global user_nonusers {		
			...
			matrix prev_estimates=r(table)'
			matselrc prev_estimates mean_se, c(1 2 5 6)
			putexcel 	A`row'="Tanzania" C`row'="FII" E`row'="Proportion of individuals who exhibit each characteristic" ///
...
putexcel set "$final_data\TZ_FII_estimates.xls", modify sheet("Country Demographics", replace)
